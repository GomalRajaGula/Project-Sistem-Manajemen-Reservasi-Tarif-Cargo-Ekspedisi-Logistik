...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id_resi'])) {
    ...
    try {
        ...
        // Delete child
        $stmtChild = $pdo->prepare("DELETE FROM kargo_reguler WHERE id_resi = ?");
        ...
        $pdo->commit();
        $_SESSION['success_message'] = "Reservasi kargo reguler dengan Resi $id_resi berhasil dihapus!";
    } catch (Exception $e) {
    ...
    header("Location: kargo_reguler.php");
    exit;
}

// Fetch regular cargos
try {
    require_once __DIR__ . '/../config/connection.php';
    $sql = "SELECT k.id_resi, k.pengirim, k.kota_tujuan, k.berat_barang, k.tarif_dasar_per_kg, k.total_tarif, k.status_packing, kr.jenis_paket, kr.estimasi_hari
            FROM kargo k
            JOIN kargo_reguler kr ON k.id_resi = kr.id_resi
            ORDER BY k.tanggal_reservasi DESC";
    $stmt = $pdo->query($sql);
    $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    error_log("kargo_reguler.php error: " . $e->getMessage());
    $data = [];
}
...
<?php if (empty($data)): ?>
<tr>
<td colspan="10" style="text-align:center; padding: 24px; color: var(--text-secondary);">Belum ada data kargo reguler</td>
</tr>
<?php else: ?>
...
<td>
    <a href="kargo_reguler.php?action=delete&id_resi=<?php echo urlencode($row['id_resi']); ?>" 
